...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <AUTORITYFILE>
            <!-- in case of multiple identities -->
            <xsl:choose>
                <xsl:when test="string-length(eac:eac-cpf/eac:multipleIdentities) != 0">
                    <xsl:for-each select="descendant::eac:cpfDescription">
                        <PERSON>
                            <xsl:apply-templates
                                select="eac:eac-cpf/eac:cpfDescription/eac:identity/eac:nameEntry"
                                mode="copy"/>
                            <xsl:apply-templates/>
                        </PERSON>
                    </xsl:for-each>
